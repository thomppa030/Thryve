...
VulkanSwapChain::VulkanSwapChain(Thryve::Core::SharedRef<Thryve::Rendering::VulkanContext> context) :
    m_renderPass(nullptr), m_swapChain(nullptr), m_swapChainImageFormat(), m_swapChainExtent()
{

    m_deviceSelector = context->GetDevice();
    m_surface = context->GetSurface();
    m_window = context->GetWindowStatic();
}
...
VulkanSwapChain::VulkanSwapChain(VulkanSwapChain&& other) noexcept :
    m_deviceSelector(other.m_deviceSelector), m_surface(other.m_surface), m_window(other.m_window),
    m_swapChain(other.m_swapChain), m_swapChainImageFormat(), m_swapChainExtent()
{
...
VulkanSwapChain& VulkanSwapChain::operator=(VulkanSwapChain&& other) noexcept
{
    ...
    m_surface = other.m_surface, m_window = other.m_window, m_deviceSelector = other.m_deviceSelector,
    m_swapChain = other.m_swapChain, m_swapChainImageFormat = other.m_swapChainImageFormat,
...
void VulkanSwapChain::InitializeSwapChain() { CreateSwapChain(); }
...
void VulkanSwapChain::CreateFramebuffers(VkDevice device)
{
    ...
    {
        std::array<VkImageView, 2> _attachments = {m_swapChainImageViews[i], m_DepthImageView};
...
std::pair<VkResult, std::optional<uint32_t>> VulkanSwapChain::AcquireNextImage(VkSemaphore imageAvailableSemaphore)
{
    uint32_t imageIndex;
    VkResult result =
        vkAcquireNextImageKHR(Thryve::Rendering::VulkanContext::GetCurrentDevice()->GetLogicalDevice(), m_swapChain,
...
void VulkanSwapChain::SetRenderPass(VkRenderPass renderPass) { m_renderPass = renderPass; }
